...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_images_path(self,videos,split):
        ...
        for index, video_path in enumerate(videos):
            ...
            if split == "train" and self.cam_view is not None and isinstance(self.cam_view,list):
                if not os.path.basename(video_images_path) in self.cam_view:
                    continue
            else:
                if split == "train" and self.cam_view is not None and os.path.basename(video_images_path) != self.cam_view and self.cam_view!="cam_all": continue


            #cam_name = video_path.split('.')[0].split("/")[-1]
            #object_folder = os.path.join(video_path.split('.')[0], "pseudo_label/object_mask") if cam_name==self.cam_view else None
            ...
            gt_mask_folder = os.path.join(self.root_dir, "gt_masks") if self.split == "test" and self.gt_mask else None
            images_path = os.listdir(image_path)
            images_path.sort()
            if self.cam_view=="cam_all" and split == "train":
                images_path = images_path[index-1::(len(videos)-1)]
            this_count = 0
            for idx, path in enumerate(images_path):
                ...
                if gt_mask_folder is not None:
                    #gt_mask_paths.append(os.path.join(gt_mask_folder, path.split('.')[0]+'.png')){index:05d}.png
                    gt_mask_paths.append(os.path.join(gt_mask_folder, f'{idx:05d}.png'))
                ...
                T = -pose[:3,3].dot(R)
                if self.cam_view=="cam_all" and split == "train":
                    image_times.append((idx*(len(videos)-1)+index-1)/countss)  # 修改
                else:
                    image_times.append(idx/countss)  # 修改
            ...
            N_time = len(images_path)
                ...
                #     video_data_save[count] = img.permute(1,2,0)
                #     count += 1
            print(f"{os.path.basename(video_images_path)}:{this_count}")
    ...
    def __getitem__(self,index):
        ...
        #print(self.gt_mask_paths is None)
