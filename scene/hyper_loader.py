...
class Load_hyper_data(Dataset):
    ...
    def __getitem__(self, index):
        if self.split == "train":
            ...
            return self.load_raw(self.i_test[index], index=index)
    ...
    def load_raw(self, idx, index=None):
        if idx in self.map.keys():
            return self.map[idx]
        camera = self.all_cam_params[idx]
        ...
        if self.split == "test" and index is not None:  #and self.all_gt_mask is not None
            gt_mask_path = f"{self.datadir}/gt_masks/{index:05d}.png"
