...
class GaussianModel:
    ...
    def set_num_classes(self, num_classes):
        ...
        # 初始化均匀概率 [N, K]
        id_prob = torch.zeros(self._xyz.shape[0], num_classes, device=self._xyz.device)
        #id_prob = id_prob / num_classes
        # 更新变量
        self.gaussian_id_prob = id_prob'''
    ...
    def densify_by_uncertain_mask(self, extent, max_screen_size, density_threshold, displacement_scale, uncertain_mask, model_path=None, iteration=None, stage=None):
        if uncertain_mask is not None:
            #self.densify_and_clone_uncertain_mask(uncertain_mask, extent, density_threshold, displacement_scale, model_path, iteration, stage)
            return self.densify_and_split_uncertain_mask(uncertain_mask, extent)
    
    def densify_and_clone_uncertain_mask(self, uncertain_mask, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):        
        # selected_pts_mask = torch.logical_and(grads_accum_mask,
        #                                      torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
        selected_pts_mask = uncertain_mask
        # breakpoint()        
        new_xyz = self._xyz[selected_pts_mask] 
        # - 0.001 * self._xyz.grad[selected_pts_mask]
        new_features_dc = self._features_dc[selected_pts_mask]
    ...
    def densify_and_split_uncertain_mask(self, uncertain_mask, scene_extent, N=2):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
        selected_pts_mask = uncertain_mask
        ...
        # 我希望大小变为原先的一半，然后位置不知道咋选
        stds = self.get_scaling[selected_pts_mask].repeat(N,1)
        means =torch.zeros((stds.size(0), 3),device="cuda")
        samples = torch.normal(mean=means, std=stds)
        rots = build_rotation(self._rotation[selected_pts_mask]).repeat(N,1,1)
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self.get_xyz[selected_pts_mask].repeat(N, 1)
        new_scaling = self.scaling_inverse_activation(self.get_scaling[selected_pts_mask].repeat(N,1) / N)  #(0.8*N)
        new_rotation = self._rotation[selected_pts_mask].repeat(N,1)
        new_features_dc = self._features_dc[selected_pts_mask].repeat(N,1,1)
        new_features_rest = self._features_rest[selected_pts_mask].repeat(N,1,1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)
        ...
        # 生成一下新高斯的 mask ，供下次使用
        new_gs_mask = torch.cat((torch.zeros((self._xyz.shape[0]- N * selected_pts_mask.sum().item()), device="cuda", dtype=bool), 
                                torch.ones(N * selected_pts_mask.sum(), device="cuda", dtype=bool)))
        return new_gs_mask


    def prune_by_mask(self, prune_mask):
        self.prune_points(prune_mask)
        torch.cuda.empty_cache()